...
//    }
    ...
    public void traversePreOrder(ListItem root) {
        if(root != null){
            System.out.print(root.getValue() + " ");
            traversePreOrder(root.previous());
            traversePreOrder(root.next());
        }
    }

    @Override
    public void traverseInOrder(ListItem root) {
        if(root != null){
            traverseInOrder(root.previous());
            System.out.print(root.getValue() + " ");
            traverseInOrder(root.next());
        }
    }
    ...
    public void traversePostOrder(ListItem root) {
        if(root != null){
            traversePostOrder(root.previous());
            traversePostOrder(root.next());
            System.out.print(root.getValue() + " ");
        }
    }

    public boolean findItem(ListItem element){
        ListItem currentItem = this.root;
        while(currentItem != null){
            int comparison = currentItem.compareTo(element);
            if(comparison < 0){
                //newItem is greater, move right if possible
                if(currentItem.next() != null){
                    currentItem = currentItem.next();
